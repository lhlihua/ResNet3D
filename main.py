...
from temporal_transforms import (LoopPadding, TemporalRandomCrop,TemporalEvenCrop,
                                 TemporalCenterCrop, SlidingWindow, TemporalSubsampling)
...
def get_opt():
    ...
    if opt.distributed:
        ...
        print(opt)
        # print(opt.result_path / 'opts.json')
        with (opt.result_path / 'opts.json').open('w') as opt_file:
            json.dump(vars(opt), opt_file, default=json_serial)
...
def get_train_utils(opt, model_parameters):
    ...
    if opt.train_crop == 'random':
        # 将图片设置为size大小 随机将原始图片变为原来的(opt.train_crop_min_scale, 1.0)倍 
        # 随机宽高比 (opt.train_crop_min_ratio, 1.0 / opt.train_crop_min_ratio)
    ...
    # 获取标准化方法 x = (x - mean) / std mean是平均值 std是标准差
    ...
    if opt.lr_scheduler == 'plateau':
        ...
        # 台阶学习率 根据里程碑降低optimizer中的学习率 下降值默认是*0.9
...
def get_predict_spatial_transform(opt):
  assert opt.predict_crop in ['center', 'nocrop']

  normalize = get_normalize_method(opt.mean, opt.std, opt.no_mean_norm,
                                    opt.no_std_norm)
  spatial_transform = [Resize(opt.sample_size)]
  if opt.predict_crop == 'center':
      spatial_transform.append(CenterCrop(opt.sample_size))
  spatial_transform.append(ToTensor())
  if opt.input_type == 'flow':
      spatial_transform.append(PickFirstChannels(n=2))
...
def main_worker(index, opt):
    ...
    # model.parameters() 通常返回的是一个模型参数的迭代器  一般传递给优化器
    ...
    if opt.inference:
        ...
        inference.inference(inference_loader, model, inference_result_path,
                            ...
                            opt.output_topk)
    predict.predictAction(model,'E:/ml_project/C3D/UCF-101/TaiChi/v_TaiChi_g07_c03.avi',get_predict_spatial_transform(opt),1)
