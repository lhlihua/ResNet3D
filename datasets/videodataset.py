...
class VideoDataset(data.Dataset):
    ...
    # 通过json文件生成dataset video_ids: video的名称例如 v_ApplyEyeMakeup_g03_c02 video_paths 视频路径
    # annotations 0000:{'label': 'ApplyEyeMakeup', 'segment': [1, 134]}
    def __make_dataset(self, root_path, annotation_path, subset,
        ...
        dataset = []
        # 将视频集合根据json 分成一个个的sample(一个视频对应一个sample)
    ...
    # 加载图片 对其进行spatial_transform compose列表中的转换 并把其转化为tensor形式的(有进行归一化和标准化)
    def __loading(self, path, frame_indices):
        clip = self.loader(path, frame_indices)
        if self.spatial_transform is not None:
            self.spatial_transform.randomize_parameters()
            clip = [self.spatial_transform(img) for img in clip]
        # 扩展维度把时间维度加入 把多帧图片品在一起
        clip = torch.stack(clip, 0).permute(1, 0, 2, 3)

        return clip
    # 获取一个视频中的一个batch_size的视频帧 通过temporal_transform compose列表中的转换 同时会调用__loading对这些视频帧进行转换
    # 返回的是转换后的视频帧 和 target
